...
**当前状态：源码与 Manual Hook 补丁检查通过；已登记全部 12 套原厂 boot 与内核配置；正在验证新内核与原厂模块的 ABI，尚未完成真机验证。没有已验证可刷写的发行版。**
完整固件编号、原厂 SHA-256 与来源见各机型 JSON。
...
项目不会自动发布或刷写未经验证的 boot。Root 构建保留事件上报，关闭冲突的 OPlus root/mount/exec 限制；保留 SELinux 配置。模块 release 字符串按原厂配置保留，真实源码提交另行记录，字符串相同并不等于模块 CRC 已匹配。
