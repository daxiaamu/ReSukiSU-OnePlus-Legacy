...
`scripts/audit.py` 对全部六款固定源码检查补丁，然后调用 `scripts/check_s3908_gesture.py` 编译实际 C 解码函数。每款测试全部 256 个事件值 × 4 组数据，共 1,024 例：检查单击类型、单击／双击起始坐标，并逐字段比较其他事件修复前后的输出。本地六款合计 6,144 例通过；同一测试已接入 Source and tooling checks Actions。

这是主机上的解码回归测试，不覆盖触控固件实际发包、休眠中断、Android AOD 策略或整机启动。本次未完成新内核的完整编译及真机单击唤醒复测。发布过的 **v0.1.0-r2 不含本次手势修复**，需重新编译并用相应原厂 boot 封装后才会生效。不能将旧镜像或其他项目的验证结果视为本项目新镜像已验证。
